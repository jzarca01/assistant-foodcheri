...
class AssistantFoodcheri {
  ...
  init(plugins) {
    ...
    if (!this.email || !this.password) {
      return Promise.reject(
        '[assistant-foodcheri] Erreur : vous devez configurer ce plugin !'
      );
    }
  ...
  async followOrder(mail, password) {
    try {
      await foodcheri.login(mail, password);
      const order = await foodcheri.getLastOrder();

      if (order.status !== this.current_step.status) {
        ...
        if (this.plugins.notifier) {
          this.plugins.notifier.action(
            `Votre livraison en est à l'étape: ${
              order.status
      ...
      if (
        ...
        setTimeout(
          async () => await this.followOrder(mail, password),
          REFRESH_DELAY
        );
      }
    } catch (err) {
  ...
  async action(commande) {
    switch (commande) {
      case 'track':
        return await this.followOrder(this.email, this.password);
      default:
        break;
    }
  }
...
exports.init = (configuration, plugins) => {
  return new AssistantFoodcheri(configuration).init(plugins).then(resource => {
